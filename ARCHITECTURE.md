...
### 2.4 Exposing the control plane

`vestad` does not terminate TLS and never binds `:443`. It sits behind `vesta-proxy` like
anything else, which means the control plane's own domain is a route in the same table, with
the same ACME machinery (§9.4) and the same certificate store.

This is not a convenience. On a single-server install — the most common shape — the control
node *is* an app server, and `vesta-proxy` already owns `:80` and `:443`. Two processes
cannot both bind them. Making `vestad` a proxied upstream is the only arrangement that works
for one server and a fleet without two different answers.

```
                    :443  vesta-proxy
                            │
      vesta.example.com ────┼──▶ 127.0.0.1:8080   vestad   (UI + REST, TLS terminated)
      api.example.com  ─────┼──▶ 172.20.0.11:3000 a container
                            │
                    :8443 ──┴──▶ vestad gRPC, mTLS, agents dial here (§6.1)
```

**Two listeners, deliberately different.** The UI and REST API listen on loopback only and
are reachable solely through the proxy. The agent gRPC endpoint is a separate port that must
be network-reachable and terminates its own mTLS — it cannot be proxied with termination,
because client-certificate verification has to happen at `vestad`. Sites that must expose
only one port can instead configure the proxy to do SNI-based **passthrough** for the agent
hostname, leaving the mTLS handshake end-to-end.

#### The control-plane route is static, not scheduled

An ordinary route reaches the proxy inside a Spec, which is generated by the control plane.
If the control plane's own route worked that way, a control plane that was down could not
serve the page explaining that it is down — and a fresh install would have no route at all
until something it cannot yet run had run.

So the agent writes the control-plane route from **local configuration at install time**, into
the proxy's initial config, independent of any Spec and independent of `vestad` being
reachable. The UI stays up to report a control plane that is unhealthy, which is precisely
when someone is looking at it.

The hostname is also **reserved in the route registry**: an app cannot claim
`vesta.example.com` and take the dashboard with it, the same way a port cannot be
double-claimed (§7.4).

#### Bootstrap, without a domain yet

A fresh install has no domain and therefore cannot obtain a certificate, but you must reach
the UI to perform first-run setup (§2.2):

1. The proxy serves the control plane on a self-signed certificate at the host's IP.
2. You reach `https://<ip>`, accept the warning, and enter the setup token from the console.
3. You set the control-plane domain; the proxy provisions a real certificate through the
   normal ACME path and the self-signed one is retired.

`install.sh --domain vesta.example.com` skips straight to step 3 when DNS is already
pointing, and the DNS preflight (§10.3) applies here too — the control plane's own
certificate is not a special case, so it fails the same legible way rather than a novel one.

#### When the proxy is down

The UI becomes unreachable while `vestad` is unaffected. The CLI reaches `vestad` directly
on its loopback port over an SSH session, and `vestad` also exposes a root-owned unix socket
for local administration. There is no state in which the control plane is running but
unmanageable.
...
**On packaging `vestad`.** Both a systemd unit and a container image are supported, and
neither affects domains or TLS (§2.4). **systemd is the documented default**, for two
reasons that are security, not taste:

- `vestad` holds the KEK and is the only process that can decrypt (above). Run as a
  container, **membership of the host's `docker` group becomes equivalent to compromising
...
**the KEK is supplied by file mount or KMS and never by environment variable** (an env var is
visible in `docker inspect`, which is the exact failure this project criticizes elsewhere),
and it should not run on a host where untrusted users hold `docker` group membership.
